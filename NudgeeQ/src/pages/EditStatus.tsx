...
export default function EditStatus() {
  ...
  const initialColorIndex = Math.max(0, COLORS.indexOf(derived.color));
  ...
  useEffect(() => {
    ...
    if (state.avatarSrc) {
      const parsed = parseAvatar(state.avatarSrc);
      const idx = COLORS.indexOf(parsed.color);
      setGender(parsed.gender);
      setSelectedColor(idx >= 0 ? parsed.color : COLORS[0]);
  ...
  useEffect(() => {
    ...
    let active = true;
    if (state?.tableId && state?.seatId && state?.avatarSrc && state?.name) {
      return;
    }
  ...
  useEffect(() => {
    setSelectedColor(COLORS[colorIndex] ?? COLORS[0]);
  }, [colorIndex]);
  ...
  const saveStatus = async () => {
    ...
    if (!tableId || !seatId || !name) {
      setError("Missing seat information. Please try again.");
      return;
    }
    ...
    try {
      ...
      setBase((prev) => ({ ...prev, avatarSrc }));
      setUser({ id: user.id, name, avatar: avatarSrc });
      nav("/nearby", { replace: true, state: { tableId } });
    } catch (e: unknown) {
      const message = e instanceof Error ? e.message : "Failed to save avatar.";
      setError(message);
    } finally {
      setSaving(false);
    }
  ...
  const title = base.name ? `${base.name}'s avatar` : "Edit avatar";
  ...
  return (
    ...
    >
      ...
      <div className="text-center w-full max-w-[1100px]">
        <h5 className="text-center font-display text-[clamp(22px,3.8vw,34px)] opacity-95">Update avatar</h5>
