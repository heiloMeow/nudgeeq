...
type Conn = { userId: string; socket: WebSocket };
const conns = new Map<string, Conn>(); // userId -> conn

export function setupWS(wss: WebSocketServer) {
  wss.on("connection", (socket, req) => {
    const url = new URL(req.url ?? "", "http://localhost");
    const userId = url.searchParams.get("userId") ?? "";
    if (!userId) { socket.close(1008, "userId required"); return; }
      ...
      try {
        ...
        */
        ...
        if (msg.type === "direct" && msg.to && typeof msg.text === "string") {
          ...
          if (peer) peer.socket.send(JSON.stringify(payload));
          // 回执给发送方
